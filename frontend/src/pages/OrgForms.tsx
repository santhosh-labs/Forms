...
export default function OrgForms() {
    ...
    const orgForms = useMemo(() =>
        allForms.filter(f => !f.isDeleted).filter(f => {
            const matchSearch = !search || f.name.toLowerCase().includes(search.toLowerCase());
            const matchType = typeFilter === 'all' || f.type === typeFilter;
            return matchSearch && matchType;
        }),
        [allForms, search, typeFilter]
    );
        ...
        <MainLayout onNewFormClick={() => { }}>
            <div className="max-w-6xl mx-auto animate-in">
                ...
                <div className="flex items-center justify-between mb-7">
                    <div>
                        <h1 className="text-4xl font-bold text-foreground tracking-tight">Organization Forms</h1>
                        <p className="text-sm text-muted-foreground mt-1">All forms accessible to everyone in your organization</p>
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-3 mb-6">
                    ...
                    <span className="text-sm text-muted-foreground">{orgForms.length} form{orgForms.length !== 1 ? 's' : ''}</span>
                </div>
                ...
                {orgForms.length === 0 ? (
                    <div className="bg-card rounded-xl border border-border shadow-sm flex flex-col items-center justify-center py-24">
                        ...
                        <h2 className="text-lg font-bold text-foreground mb-1">No organization forms found</h2>
                        <p className="text-sm text-muted-foreground">Forms created by org members will appear here.</p>
                    </div>
                ) : viewMode === 'grid' ? (
                    /* Grid */
